...
import { switchHomeTheme } from "./lib/home.js";
...
function parseArguments(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    else if (args[index] === "--home") options.home = true;
  ...
  if (Boolean(options.publicationId) === Boolean(options.home)) {
    throw new Error("必须且只能指定 --home 或 --publication");
  }
  return options;
}
...
try {
  ...
  if (options.home) {
    const result = await switchHomeTheme(rootDir, options.theme, options);
    console.log(JSON.stringify({ ...result, target: "home" }));
  } else {
