...
function fail(filePath, field, message) {
  throw new ContentValidationError(filePath, field, message);
}
...
function requireString(value, filePath, field) {
  if (typeof value !== "string" || value.trim() === "") fail(filePath, field, "必须是非空字符串");
}
...
function requireIsoTime(value, filePath, field) {
  if (
    ...
    || !isValidContentDate(value.slice(0, 10))
...
async function validateImage(image, filePath, field, validateAsset) {
  ...
  await validateAsset?.(image.src, filePath, `${field}.src`);
  ...
  if (image.sourceUrl !== undefined) requireHttpUrl(image.sourceUrl, filePath, `${field}.sourceUrl`);
}
...
function validateCoverage(coverage, filePath) {
  ...
  if (Date.parse(coverage.start) >= Date.parse(coverage.end)) fail(filePath, "coverage", "start 必须早于 end");
}

async function validateItems(items, filePath, schemaVersion, strictCandidate, validateAsset) {
  if (!Array.isArray(items) || items.length === 0) fail(filePath, "items", "必须是非空数组");
  const ids = new Set();
  for (const [index, item] of items.entries()) {
    const field = `items[${index}]`;
    requireObject(item, filePath, field);
    if (strictCandidate) requireAllowedFields(item, ITEM_FIELDS_V2, filePath, field);
    requireString(item.id, filePath, `${field}.id`);
    if (!ITEM_ID_PATTERN.test(item.id)) fail(filePath, `${field}.id`, "只能包含小写字母、数字和连字符，且不能以连字符开头或结尾");
    ...
    if (!PRIORITIES.has(item.editorial.priority)) fail(filePath, `${field}.editorial.priority（内容 ${item.id}）`, "只能是 lead、important 或 normal");
    ...
    if (item.image !== undefined) {
      ...
      await validateImage(item.image, filePath, `${field}.image`, validateAsset);
    }
    if (item.score !== undefined || item.selected !== undefined) fail(filePath, `${field}（内容 ${item.id}）`, "不能包含 AIHot 的 score 或 selected 字段");
    if (!Array.isArray(item.sources) || item.sources.length === 0) fail(filePath, `${field}.sources（内容 ${item.id}）`, "必须是非空数组");
    ...
    for (const [sourceIndex, source] of item.sources.entries()) {
      ...
      if (sourceUrls.has(source.url)) fail(filePath, `${sourceField}.url（内容 ${item.id}）`, "同一条内容内不能重复");
      sourceUrls.add(source.url);
      if (source.originalTitle !== undefined) requireString(source.originalTitle, filePath, `${sourceField}.originalTitle（内容 ${item.id}）`);
      for (const timeField of ["publishedAt", "discoveredAt"]) {
        if (source[timeField] !== undefined) requireIsoTime(source[timeField], filePath, `${sourceField}.${timeField}（内容 ${item.id}）`);
      }
...
export async function validateSiteValue(site, options = {}) {
  const filePath = options.filePath ?? "site";
  requireObject(site, filePath, "$");
  requireString(site.name, filePath, "name");
  if (typeof site.accentColor !== "string" || !COLOR_PATTERN.test(site.accentColor)) fail(filePath, "accentColor", "必须是六位十六进制颜色");
  if (site.logo !== undefined) {
    requireAssetPath(site.logo, filePath, "logo");
    await options.validateAsset?.(site.logo, filePath, "logo");
  }
  requireObject(site.priorityLimits, filePath, "priorityLimits");
  for (const key of Object.keys(site.priorityLimits)) {
    if (!PRIORITY_LIMIT_FIELDS.has(key)) fail(filePath, `priorityLimits.${key}`, "不是支持的优先级");
  }
  for (const priority of PRIORITIES) {
    const limit = site.priorityLimits[priority];
    if (limit !== null && (!Number.isInteger(limit) || limit < 0)) fail(filePath, `priorityLimits.${priority}`, "必须是大于等于 0 的整数或 null（不限）");
  }
  return site;
}

export async function validateIssueValue(issue, options = {}) {
  const filePath = options.filePath ?? "Issue";
  if (!isValidContentDate(options.expectedDate)) fail(filePath, "目标日期", "必须是合法的 YYYY-MM-DD");
  requireObject(issue, filePath, "$");
  if (issue.schemaVersion !== 1 && issue.schemaVersion !== 2) fail(filePath, "schemaVersion", "必须等于 1 或 2");
  if (issue.date !== options.expectedDate) fail(filePath, "date", "必须与文件名一致");
  requireIsoTime(issue.generatedAt, filePath, "generatedAt");
  validateCoverage(issue.coverage, filePath);
  if (!Number.isInteger(issue.revision) || issue.revision < 1) fail(filePath, "revision", "必须是大于等于 1 的整数");
  await validateItems(issue.items, filePath, issue.schemaVersion, false, options.validateAsset);
  return issue;
}

export async function validateCandidateValue(candidate, options = {}) {
  const filePath = options.filePath ?? "Candidate";
  if (!isValidContentDate(options.expectedDate)) fail(filePath, "文件名", "必须是合法的 YYYY-MM-DD.json");
  requireObject(candidate, filePath, "$");
  requireAllowedFields(candidate, CANDIDATE_FIELDS, filePath, "$");
  if (candidate.schemaVersion !== 1 && candidate.schemaVersion !== 2) fail(filePath, "schemaVersion", "必须等于 1 或 2");
  if (candidate.date !== options.expectedDate) fail(filePath, "date", "必须与文件名一致");
  requireIsoTime(candidate.generatedAt, filePath, "generatedAt");
  validateCoverage(candidate.coverage, filePath);
  await validateItems(candidate.items, filePath, candidate.schemaVersion, true, options.validateAsset);
  return candidate;
}
