...
function assertText(value, field, maximum, { nullable = false } = {}) {
  ...
  if (codePointLength(trimmed) > maximum) throw new TodoError(field, `不能超过 ${maximum} 个字符`);
}
...
export function validateTodoConfig(value, source = "config/todo.json") {
  assertObject(value, source);
  assertExactKeys(value, ["schemaVersion", "enabled"], source);
  if (value.schemaVersion !== 1) throw new TodoError(`${source}.schemaVersion`, "必须是整数 1");
  if (typeof value.enabled !== "boolean") throw new TodoError(`${source}.enabled`, "必须是布尔值");
  return value;
}

export function validateTodoCandidate(value, source = "Todo Candidate") {
  assertObject(value, source);
  assertExactKeys(value, ["schemaVersion", "candidateId", "generatedAt", "baseRevision", "operations"], source);
  if (value.schemaVersion !== 1) throw new TodoError(`${source}.schemaVersion`, "必须是整数 1");
  if (typeof value.candidateId !== "string" || !candidateIdPattern.test(value.candidateId)) {
    throw new TodoError(`${source}.candidateId`, "只能使用小写字母、数字和连字符");
  }
  assertTimestamp(value.generatedAt, `${source}.generatedAt`);
  if (!Number.isInteger(value.baseRevision) || value.baseRevision < 0) {
    throw new TodoError(`${source}.baseRevision`, "必须是非负整数");
  }
  if (!Array.isArray(value.operations) || value.operations.length === 0) {
    throw new TodoError(`${source}.operations`, "必须是非空数组");
  }
...
export function validateTodoState(value, source = "todo/data/state.json") {
  assertObject(value, source);
  assertExactKeys(value, ["schemaVersion", "revision", "updatedAt", "items"], source);
  if (value.schemaVersion !== 1) throw new TodoError(`${source}.schemaVersion`, "必须是整数 1");
  if (!Number.isInteger(value.revision) || value.revision < 0) {
    throw new TodoError(`${source}.revision`, "必须是非负整数");
  }
  assertTimestamp(value.updatedAt, `${source}.updatedAt`, { nullable: true });
  if (!Array.isArray(value.items)) throw new TodoError(`${source}.items`, "必须是数组");
  if (value.revision === 0 && (value.updatedAt !== null || value.items.length !== 0)) {
    throw new TodoError(source, "revision 0 只能表示空状态");
  }
  if (value.revision > 0 && value.updatedAt === null) {
    throw new TodoError(`${source}.updatedAt`, "有效写入后不能为空");
  }
  const ids = new Set();
  value.items.forEach((item, index) => {
    const field = `${source}.items.${index}`;
