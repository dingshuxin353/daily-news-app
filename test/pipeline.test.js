...
async function writeCandidate(target, candidate) {
  const candidatePath = path.join(target, "data", "candidates", `${candidate.date}.json`);
  await writeFile(candidatePath, `${JSON.stringify(candidate, null, 2)}\n`, "utf8");
  return candidatePath;
}

async function readIssue(target, date) {
  return readJson(path.join(target, "data", "issues", `${date}.json`));
}

async function snapshot(target, date) {
  const files = [
    path.join(target, "data", "issues", `${date}.json`),
    path.join(target, "data", "compiled", `${date}.json`),
    path.join(target, "data", "index.json"),
  ];
...
test("新日期返回 created，并统一写入 issue、compiled 和 index", async () => {
  ...
  const result = await processCandidate(target, candidatePath, { today: "2026-08-20" });
  const issue = await readIssue(target, "2026-08-20");
  const compiled = await readJson(path.join(target, "data", "compiled", "2026-08-20.json"));
  const index = await readJson(path.join(target, "data", "index.json"));
...
test("replace 只有显式模式才删除候选未包含的旧内容", async () => {
  ...
  const result = await processCandidate(target, candidatePath, {
    today: "2026-08-19",
    mode: "replace",
  });
  const issue = await readIssue(target, "2026-08-19");
...
test("无业务变化保留 generatedAt 和 revision，仅修复过期 compiled 与 index", async () => {
  ...
  const candidatePath = await writeCandidate(target, candidate);
  await writeFile(
    path.join(target, "data", "compiled", "2026-08-19.json"),
    "{invalid",
    "utf8",
  );
  await writeFile(
...
test("coverage 改变或匹配歧义时拒绝并保持全部正式产物不变", async () => {
  ...
  await assert.rejects(
    () => processCandidate(target, candidatePath, { today: "2026-08-19" }),
    /coverage.*固定采集窗口/,
  );
...
test("历史日期未授权和同日期锁冲突均拒绝写入", async () => {
  ...
  await assert.rejects(
    () => processCandidate(target, candidatePath, { today: "2026-08-20" }),
    /--allow-history/,
  );

  const lockDir = path.join(target, "data", ".locks");
...
test("候选必须位于 data/candidates 目录", async () => {
  ...
  await assert.rejects(
    () => processCandidate(target, outsidePath, { today: "2026-08-19" }),
    /必须位于 data\/candidates/,
  );
});
