...
function ThemePreview({ theme, compact = false }: { theme: BrowserTheme; compact?: boolean }) {
  ...
  return <figure className={compact ? "m51-theme-preview is-compact" : "m51-theme-preview"} style={style} aria-label={`${theme.name} 主题预览`}>
    <div className="m51-theme-preview__masthead"><strong>DailyNews</strong><time>示例</time></div>
    <div className="m51-theme-preview__story">
      <small>日报预览</small>
      <h3>先看重要内容</h3>
      <p>按阅读顺序整理几条内容。</p>
    </div>
    <figcaption>私人日报页面预览</figcaption>
  </figure>;
}
...
function ThemeChoices(input: {
  ...
  return <fieldset className="m51-theme-choices">
    <legend>主题</legend>
    <p>{input.allowInherit ? "选择后只影响这个日报站点的阅读页面。" : "选择后会影响 Home 及跟随 Home 的日报站点。"}</p>
...
export function SitesPage(input: {
  ...
  const updates: Record<string, string> = {
    ...
    disabled: "日报站点已停用；已有日报仍可阅读。",
    restored: "日报站点已恢复，并排在启用列表末尾。",
    "todo-disabled": "Personal Todo 已关闭；已有待办仍会保留。",
  };
  const instruction = input.created
    ? `请继续使用已有的 DailyNews 连接，为“${input.created.name}”（私有地址 /p/${input.created.publicationId}/）设置长期关注主题和更新时间，并生成第一份日报供我确认。`
    : null;
  return <SettingsLayout basePath={input.basePath} shell={input.shell} current="sites" title="日报站点" kicker="日报站点" summary="Home 固定在最前；启用中的日报按这里的顺序排列，第一项就是 Agent 默认写入目标。">
    <Status>{input.reason === "todo-disabled" ? "Personal Todo 还没开启。请在页面底部开启。" : input.updated ? updates[input.updated] : undefined}</Status>
    ...
    <section className="m51-settings-section m51-todo-setting" id="personal-todo" tabIndex={-1}>
      <div><p className="m51-kicker">固定能力</p><h2>Personal Todo</h2><p>{input.snapshot.todo.enabled ? "Agent 可以读取和提交个人待办；关闭后已有任务仍会保留。" : "默认关闭；开启后 Agent 才能保存个人待办。"}</p><p>{input.snapshot.todo.hasFormalData ? "已有待办；本页不显示任务内容。" : "还没有待办。"}</p></div>
      <div className="m51-site-actions">{input.snapshot.todo.enabled ? <><Button className="m51-button" label="关闭" variant="destructive" size="md" href={appPath(input.basePath, "/settings/sites/todo/disable")} /><a className="m51-text-link" href={appPath(input.basePath, "/todo/")}>查看 <IconExternalLink size={16} aria-hidden="true" /></a></> : <form method="post" action={appPath(input.basePath, "/settings/sites/todo/enable")}><input type="hidden" name="_csrf" value={input.csrfToken} /><Button className="m51-button" label="开启" variant="primary" size="md" type="submit" /></form>}</div>
    </section>
  </SettingsLayout>;
}
...
export function HomeSettingsPage(input: { basePath: string; shell: ReadingShell; snapshot: SiteManagementSnapshot; themes: BrowserTheme[]; csrfToken: string; name?: string; themeId?: string; error?: string; saved?: boolean }) {
  ...
  return <SettingsLayout basePath={input.basePath} shell={input.shell} current="sites" title="设置 Home" kicker="Home" summary="Home 是私人编辑部的固定首页，名称和主题会影响所有跟随 Home 的日报站点。">
    ...
    <form className="m51-settings-form m51-settings-section" method="post" action={appPath(input.basePath, "/settings/sites/home")}>
      ...
      <Button className="m51-button" label="保存 Home 设置" variant="primary" size="lg" type="submit" />
    </form>
  </SettingsLayout>;
}
...
export function PublicationFormPage(input: { basePath: string; shell: ReadingShell; themes: BrowserTheme[]; csrfToken: string; mode: "new" | "edit"; publication?: ManagedPublication; name?: string; publicationId?: string; theme?: ThemeSelection; error?: string; saved?: boolean }) {
  ...
  const title = edit ? `设置 ${input.publication?.name ?? "日报站点"}` : "新建日报站点";
  return <SettingsLayout basePath={input.basePath} shell={input.shell} current="sites" title={title} kicker={edit ? "日报站点" : "新建日报"} summary={edit ? "私有地址创建后保持不变；名称与主题可以一起保存。" : "先建立一个空站点，再把设置说明交给已有 Agent。"}>
...
export function AccountSettingsPage(input: { basePath: string; shell: ReadingShell; csrfToken: string; profile: UserProfile; nickname?: string; error?: string; saved?: boolean }) {
  ...
  return <SettingsLayout basePath={input.basePath} shell={input.shell} current="account" title="账户与安全" kicker="账户与安全" summary="管理账户称呼和当前登录会话。Agent 授权在单独的栏目中管理。">
...
export function AdvancedAccessPage(input: { basePath: string; shell: ReadingShell; apiBaseUrl: string; mcpUrl: string }) {
  return <SettingsLayout basePath={input.basePath} shell={input.shell} current="advanced" title="高级接入" kicker="高级接入" summary="为脚本或高级客户端提供接口地址和 OpenAPI 说明。Token 统一在 Agent 授权中管理。">
